...
def load_data_Charles(image_path, config):
    ...
    input_sets = data_pairs[:, :, 0:config.INPUT_C]
...
def load_data_Charles_test(image_path, config):
    ...
    # input_sets = np.ascontiguousarray(np.flip(input_sets, axis=1))
    # input_sets = np.ascontiguousarray(np.flip(input_sets, axis=0))
    label_sets = data_pairs[:, :, config.LABEL_C-1:config.LABEL_C]
