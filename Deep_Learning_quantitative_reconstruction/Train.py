...
def main(config):
    ...
    brain_batch = get_loader(data_dir, config, crop_key=False,num_workers=1, shuffle=False, mode='brain1')
            ...
            for i,(images, GT) in enumerate(train_batch):
                ...
                if total_iters%config.step == 0:
                    ...
                    with torch.no_grad():
                     ...
                     ## ********************************************test***********************************************#
                     for i, (brain_images, GT) in enumerate(brain_batch):
                        ...
                        save_torch_result(SR_brain[:, 0:1, :, :], save_dir,
                                          format='png', cmap='jet', norm=False, crange=[0 ,4])
                     net.train()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--LABEL_C', type=int, default=5)
