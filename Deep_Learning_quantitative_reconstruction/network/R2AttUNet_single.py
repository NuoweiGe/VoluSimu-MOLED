...
class Inference(nn.Module):
    def __init__(self,img_ch=3, output_ch=1,filter_dim=64, t=2):
        super(Inference, self).__init__()

        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.Upsample = nn.Upsample(scale_factor=2)

        self.RRCNN1 = RRCNN_block(ch_in=img_ch, ch_out=filter_dim, t=t)
        ...
        self.Conv1 = outconv(64, output_ch)
    ...
    def forward(self, x):
        ...
        d5 = self.Up5(x5)
        #print(numpy.shape(d5),numpy.shape(x4))
        x4 = self.Att5(d5, x4)
...
class loss_fun_total(nn.Module):
    ...
    def forward(self, x, y):
        loss1 = torch.mean(torch.pow((x[:,0:1,:,:] - y[:,2:3,:,:]), 2))
        return loss1
